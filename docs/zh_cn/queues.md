...
- [简介](#introduction)
    ...
    - [驱动说明和前提条件](#driver-prerequisites)
- [创建任务](#creating-jobs)
...
- [分发任务](#dispatching-jobs)
    ...
    - [在分派前准备任务](#preparing-jobs-before-dispatch)
    - [任务和数据库事务](#jobs-and-database-transactions)
    - [任务链](#job-chaining)
    - [自定义队列和连接](#customizing-the-queue-and-connection)
    - [指定最大任务尝试次数/超时值](#max-job-attempts-and-timeout)
    - [SQS FIFO 和公平队列](#sqs-fifo-and-fair-queues)
    - [队列故障转移](#queue-failover)
    - [错误处理](#error-handling)
- [任务批处理](#job-batching)
    - [定义可批处理任务](#defining-batchable-jobs)
    - [分发批次](#dispatching-batches)
    - [链和批次](#chains-and-batches)
...
- [运行队列工作器](#running-the-queue-worker)
    - [`queue:work` 命令](#the-queue-work-command)
    - [队列优先级](#queue-priorities)
    - [队列工作器和部署](#queue-workers-and-deployment)
    - [响应工作器信号](#reacting-to-worker-signals)
    - [任务过期和超时](#job-expirations-and-timeouts)
    - [暂停和恢复队列工作器](#pausing-and-resuming-queue-workers)
...
- [测试](#testing)
    - [伪造任务子集](#faking-a-subset-of-jobs)
...
在构建 Web 应用时，你可能会有些任务（如解析和存储上传的 CSV 文件）在典型的 Web 请求中执行时间过长。幸运的是，Laravel 允许你轻松创建可在后台处理的队列任务。通过将耗时任务移到队列中，你的应用可以极快地响应 Web 请求，并为客户提供更好的用户体验。

Laravel 队列为各种不同的队列后端提供了统一的队列 API，例如 [Amazon SQS](https://aws.amazon.com/sqs/)、[Redis](https://redis.io) 甚至关系数据库。

Laravel 的队列配置选项存储在应用的 `config/queue.php` 配置文件中。在此文件中，你将找到框架包含的每个队列驱动的连接配置，包括数据库、[Amazon SQS](https://aws.amazon.com/sqs/)、[Redis](https://redis.io) 和 [Beanstalkd](https://beanstalkd.github.io/) 驱动，以及一个将立即执行任务的同步驱动（用于开发或测试）。还包含一个 `null` 队列驱动，它会丢弃队列任务。

> [!NOTE]
> Laravel Horizon 是一个适用于 Redis 队列的漂亮仪表板和配置系统。查看完整的 [Horizon 文档](/docs/{{version}}/horizon)了解更多信息。

<a name="connections-vs-queues"></a>
### 连接 vs. 队列

在开始使用 Laravel 队列之前，理解"连接"和"队列"之间的区别非常重要。在 `config/queue.php` 配置文件中，有一个 `connections` 配置数组。此选项定义了与后端队列服务（如 Amazon SQS、Beanstalk 或 Redis）的连接。但是，任何给定的队列连接可能有多个"队列"，这些队列可以被视为不同的堆栈或队列任务集合。

请注意，`queue` 配置文件中的每个连接配置示例都包含一个 `queue` 属性。这是任务在发送到给定连接时将被分派到的默认队列。换句话说，如果你在分派任务时没有明确定义应分派到哪个队列，则该任务将放置在连接配置的 `queue` 属性中定义的队列上：
...
有些应用可能永远不需要将任务推送到多个队列，而是更喜欢有一个简单的队列。但是，将任务推送到多个队列对于希望对任务处理方式进行优先级排序或分段的应用程序特别有用，因为 Laravel 队列工作器允许你按优先级指定它应处理的队列。例如，如果你将任务推送到 `high` 队列，你可以运行一个赋予它们更高处理优先级的工作器：
...
为了使用 `database` 队列驱动，你需要一个数据库表来保存任务。通常，这包含在 Laravel 默认的 `0001_01_01_000002_create_jobs_table.php` [数据库迁移](/docs/{{version}}/migrations)中；但是，如果你的应用不包含此迁移，你可以使用 `make:queue-table` Artisan 命令来创建它：
...
为了使用 `redis` 队列驱动，你应在 `config/database.php` 配置文件中配置一个 Redis 数据库连接。
...
如果你的 Redis 队列连接使用 [Redis 集群](https://redis.io/docs/latest/operate/rs/databases/durability-ha/clustering)，你的队列名称必须包含一个[键哈希标签](https://redis.io/docs/latest/develop/using-commands/keyspace/#hashtags)。这是为了确保给定队列的所有 Redis 键都放置在同一个哈希槽中：
...
使用 Redis 队列时，你可以使用 `block_for` 配置选项来指定驱动在遍历工作器循环并重新轮询 Redis 数据库之前应等待任务变为可用的时间。

根据队列负载调整此值比持续轮询 Redis 数据库以获取新任务更高效。例如，你可以将该值设置为 `5`，表示驱动在等待任务变为可用时应阻塞五秒：
...
> [!WARNING]
> 将 `block_for` 设置为 `0` 将导致队列工作器无限期阻塞，直到有可用任务。这还会阻止处理诸如 `SIGTERM` 等信号，直到下一个任务被处理。

<a name="sqs-overflow-storage"></a>
#### SQS 溢出存储

Amazon SQS 限制了队列消息有效负载的最大大小。如果你需要分派有效负载可能超过此限制的任务，你可以配置 Laravel 将过大的 SQS 有效负载存储在缓存存储中，并通过 SQS 发送指针。要启用此功能，请向 SQS 队列连接配置添加一个 `overflow` 数组：
...
启用溢出存储后，Laravel 会将至少 1 MB 的有效负载存储在配置的缓存存储中。如果 `always` 选项为 `true`，则无论大小如何，每个 SQS 有效负载都将存储在缓存存储中。由于队列任务在处理时需要从缓存存储中检索其有效负载，你应选择一个能够保留有效负载直到工作器处理它们的存储。默认情况下，存储的有效负载在其任务成功处理并从 SQS 删除后被删除。

如果 `flush_on_clear` 选项为 `true`，则当 `queue:clear` 命令清除 SQS 队列时，配置的溢出缓存存储将被刷新。由于刷新缓存存储可能移除该存储中的所有项目，你应在启用此选项时配置 SQS 溢出存储使用专用的缓存存储。
...
默认情况下，应用中所有可队列化任务都存储在 `app/Jobs` 目录中。如果 `app/Jobs` 目录不存在，它将在你运行 `make:job` Artisan 命令时创建：

```shell
php artisan make:job ProcessPodcast
```

生成的类将实现 `Illuminate\Contracts\Queue\ShouldQueue` 接口，向 Laravel 指示该任务应被推送到队列以异步运行。

> [!NOTE]
> 任务存根可以使用[存根发布](/docs/{{version}}/artisan#stub-customization)进行自定义。

<a name="class-structure"></a>
### 类结构

任务类非常简单，通常只包含一个 `handle` 方法，该方法在队列处理任务时调用。首先，让我们看一个示例任务类。在此示例中，我们假设管理一个播客发布服务，需要在播客发布前处理上传的播客文件：
...
在此示例中，请注意我们能够直接将 [Eloquent 模型](/docs/{{version}}/eloquent)传递到队列任务的构造函数中。由于任务使用的 `Queueable` trait，Eloquent 模型及其加载的关系将在任务处理时被优雅地序列化和反序列化。

如果你的队列任务在其构造函数中接受一个 Eloquent 模型，则只有模型的标识符会被序列化到队列中。当任务实际被处理时，队列系统将自动从数据库中重新检索完整的模型实例及其加载的关系。这种模型序列化方法允许向队列驱动发送更小的任务负载。

<a name="handle-method-dependency-injection"></a>
#### `handle` 方法依赖注入

`handle` 方法在队列处理任务时被调用。请注意，我们能够在任务的 `handle` 方法上类型提示依赖项。Laravel [服务容器](/docs/{{version}}/container)自动注入这些依赖项。

如果你希望完全控制容器如何将依赖项注入到 `handle` 方法中，你可以使用容器的 `bindMethod` 方法。`bindMethod` 方法接受一个回调，该回调接收任务和容器。在回调中，你可以自由地以你希望的任何方式调用 `handle` 方法。通常，你应从 `App\Providers\AppServiceProvider` [服务提供者](/docs/{{version}}/providers)的 `boot` 方法中调用此方法：
...
> [!WARNING]
> 二进制数据（如原始图像内容）在传递给队列任务之前应通过 `base64_encode` 函数传递。否则，任务在放置到队列时可能无法正确序列化为 JSON。

<a name="handling-relationships"></a>
#### 队列化关系

由于所有加载的 Eloquent 模型关系也会在任务队列化时被序列化，序列化后的任务字符串有时会变得相当大。此外，当任务被反序列化并从数据库中重新检索模型关系时，它们将完整地被检索。在任务队列化过程中，模型序列化之前应用的任何先前关系约束都不会在任务反序列化时被应用。因此，如果你希望处理给定关系的子集，你应在队列任务中重新约束该关系。

或者，为了防止关系被序列化，你可以在设置属性值时在模型上调用 `withoutRelations` 方法。此方法将返回一个不带加载关系的模型实例：
...
有时，你可能希望确保在任何时间点上队列中只有一个特定任务的实例。你可以通过在任务类上实现 `ShouldBeUnique` 接口来实现。此接口不要求你在类上定义任何额外的方法：
...
有时，你可能希望确保在短时间内多次分发同一任务时，只有最后一次分发实际执行。你可以通过向任务添加 `DebounceFor` 属性来实现：
